...
- **引用付き回答 + 三段の引用検証**: 回答中の `[S1][S2]` をクリックで原文へジャンプ。(1) 存在しないソース番号を検出、(2) 引用文がソース本文に字句的に裏付けられていれば「根拠確認済み」と表示、(3) 引用文が**別の**ソースに強く一致する場合は番号の取り違えとして検出。字句信号の限界(同義語言い換えは判定不能)を踏まえ、確信できる場合のみ提示し正しい回答を誤って咎めない。LLM不要・依存ゼロ
