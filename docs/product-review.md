# Shoin プロダクトレビュー — 長所・短所・改善案 (v0.2.180 時点、2026-09-14 更新)

shoin/ 15モジュール + 単一HTML UI・テスト715件・ドキュメント一式の精査結果。
...
| 5 | **ブラウザ描画の自動テストは無い** | レンダリング・レイアウト・イベント配線の回帰はライブ検証のみ。ただし静的契約テスト(`tests/test_ui_contract.py`, v0.2.154)が JS構文・data-i18n の ja/en 完備・UIが叩く `/api/…` のルート実在を恒久カバーし、3件とも故意の破壊で捕捉を実証済み。この限界の実害も2件確認: `SHOIN_LANG` が Web UI に一切効いていなかった欠陥(v0.2.177)と、最後のノートブック削除後にエクスポートリンクが削除済みIDを指したまま残る欠陥(v0.2.179)は、いずれも静的契約テストの対象外(実行時の状態遷移)で、実ブラウザ(Playwright)での偶発的観察・意図的再現でのみ見つかった — 恒久カバーの穴が実際に欠陥を通した実例であり、静的契約テストで解決する範囲ではない |
