...
- **`_embed_chunks()` が部分的な LLM 失敗時に `embed_model` 設定を更新せず、以降のモデル変更警告が発火しない**: 埋め込みバッチ途中で `LLMError` が発生した場合、一部のチャンクへの埋め込みは成功しているが `set_setting("embed_model", ...)` が `return done` の前に呼ばれないままだった。その後ユーザーが `SHOIN_EMBED_MODEL` を変更して新しいソースをインデックスした際、DB に保存済みのモデル名が None のままであるため「埋め込みモデルが変わった」警告が表示されず、混在した埋め込みが無音で許容されてしまっていた。`except LLMError` を `pass` に変更し、ループ後に `done > 0` の場合のみ `set_setting` を呼ぶよう修正。完全失敗 (done=0) の場合は記録しない。

## [v0.1.51] - 2026-06-14
### Fixed
- **v0.1.49 リグレッション: `_is_cjk_word()` が `々` (U+3005, 踊り字) を語境界として扱い `人々の生活` などの検索クエリを分断する**: v0.1.49 で U+3000–U+303F 全体を CJK 語構成文字から除外したが、このブロックには句読点 (。、　) だけでなく踊り字 `々` も含まれる。`々` は "人々"・"様々"・"国々" など日本語の語彙に不可欠な形態素であり語境界ではない。`query_terms("人々の生活")` が ["人", "の生活"] を返すようになり `人々` を含む FTS クエリが生成されず、この形を含む文書がマッチしない問題が発生していた。`_is_cjk_word()` 内で `cp == 0x3005` を例外として True を返すよう修正し、それ以外の U+3000–U+303F は引き続き語境界として扱う。
- **`cosine()` が NaN / Inf を含むベクターに対して NaN を返し `sort()` の動作が未定義になる**: 壊れた LLM エンドポイントが NaN や Inf 値を含む埋め込みベクターを返した場合、`cosine()` の演算結果が NaN になり、`vector_search()` 内の `hits.sort(key=lambda h: h.vec)` が未定義動作を起こす可能性があった。`math.isfinite(result)` チェックを追加し、非有限値のコサイン類似度を 0.0 にフォールバック。
- **`vector_search()` の型注釈が `None` を許容せず防御的ガードが欠如**: `retrieve()` は `query_vec` が None の場合 `vector_search` を呼ばないが、関数自体には None ガードがなかった。型注釈を `list[float] | None` に変更し `if not query_vec: return []` を追加。将来の直接呼び出しでの誤った `None` 渡しによるクラッシュを防止。

## [v0.1.50] - 2026-06-14
### Fixed
- **`_h_ask_sse()` が LLM ミッドストリームエラー時に `parts = [text]` で既受信トークンを破棄し、DB と実際のクライアント表示が乖離する**: ネットワーク切断やタイムアウトで LLM が数トークン送信後に失敗した場合、`parts` には既に受信済みトークンが入っているが `parts = [text]` がそれを上書きしていた。クライアントは「受信済みトークン + デグレード前置テキスト」を表示するが、DB には「デグレード前置テキストのみ」が保存され、チャット履歴が実際の表示と食い違う状態になっていた。`parts = [text]` を `parts.append(text)` に変更し、DB に保存されるメッセージがクライアントが受信した全コンテンツと一致するよう修正。
- **`suggest_questions()` に NFKC 正規化後は常に False になるデッドコードが存在する**: `q = unicodedata.normalize("NFKC", ...)` により全角疑問符 `？` → ASCII `?`、全角感嘆符 `！` → `!` に変換されるため、`"？" in q`・`q_base.rstrip("。．!！?？")` の `！？` は到達不能コードだった。`q_base.rstrip("。．!?")` に整理し、条件式を `"?" in q or q_base.endswith("か")` に簡略化。
...
- **`_CJK_RANGES` が CJK 記号・句読点ブロック (U+3000–U+303F) を含まずトークン推定が過小評価される**: `。` (句点 U+3002)・`、` (読点 U+3001)・`　` (全角スペース U+3000)・`々` (踊り字 U+3005) 等はそれぞれ LLM トークナイザ上 1 トークンを消費するが `is_cjk()` が `False` を返していた。このため `estimate_tokens("書院。")` は 2 (実際は 3) を返し、`_hard_split()` のチャンク境界が CJK 句読点密度の高いテキストで最大トークン制限を超過する可能性があった。U+3000–U+303F 範囲を `_CJK_RANGES` に追加し、`。`・`、`・`　` 等を正しくトークンとしてカウント。

## [v0.1.47] - 2026-06-14
### Fixed
- **`_h_ask_sse()` が "meta" SSE イベント送信失敗時に空文字列のアシスタントメッセージを DB に保存する**: クライアントがリクエスト送信直後に切断した場合 (モバイル回線切断など)、`ConnectionError` が "meta" イベントで発生し `store.add_message(nb_id, "assistant", "", "{}")` が実行されていた。空のアシスタントメッセージが会話履歴に永続化され、再接続後の `history_messages()` で孤立したユーザーターンとして表示される問題があった。`except ConnectionError: return` に変更し、有用な内容のないメッセージが保存されないよう修正。孤立したユーザーメッセージは既存の `history_messages()` の末尾ユーザーターン除去ロジックで正しく処理される。
- **`_h_ask_sse()` が LLM ストリーミング開始前にクライアント切断した場合にも空のアシスタントメッセージを保存する**: "meta" 送信後・トークン到達前に `ConnectionError` が発生した場合、`parts = []` → `full = ""` となり `store.add_message` が空文字列で呼ばれていた。`if full:` ガードを追加し、有効なコンテンツが存在する場合のみメッセージを保存するよう修正。

## [v0.1.46] - 2026-06-14
### Fixed
- **`export_ris()` が RIS エントリ間に空行を挿入しない**: RIS 2001 仕様では各 `ER  - ` タグの後に空行が必要だが、`"\n".join(entries)` は単一の改行しか挿入しないため、Zotero や Mendeley 等の RIS パーサーがエントリ境界を誤認識することがあった。`"\n\n".join(entries)` に変更し、`BibTeX` の既存の動作と一致させた。
- **`_ris_escape()` と `_bib_escape()` が `\r\n`/`\r` 行末を二重スペースに変換する**: Windows 環境から登録したソース (テキストファイルや HTML) がタイトルや origin に CRLF (`\r\n`) を含む場合、`replace("\n", " ").replace("\r", " ")` の二段階置換が `"  "` (ダブルスペース) を生成していた。`" ".join(text.splitlines())` に変更し、`\r\n` / `\r` / `\n` を含むすべての行末形式を単一スペースに正規化。

## [v0.1.45] - 2026-06-14
### Fixed
- **`_minmax()` がすべてのスコアが 0 の場合に `[1.0, 1.0, ...]` を返しゼロ関連性ヒットのフュージョンスコアを誤って最大化する**: SQLite FTS5 の BM25 は全チャンクに出現するクエリ語 (IDF=0) に対しスコア 0.0 を返す。`_minmax([0.0, 0.0, ...])` は `hi - lo < 1e-12` 条件 (等値) に該当し `[1.0, 1.0, ...]` を返していた。この結果、ゼロ関連性の BM25 ヒットがフュージョン時に最大 BM25 重み (例: `(1-alpha) * 1.0 = 0.3`) を受け取り、真のベクトルヒットより高スコアになる可能性があった。`lo < 1e-12` のとき 0.0、そうでなく等値なら 1.0 (同率) を返すよう修正。
- **`_overlap()` が空クレームに `1.0`(恒真) を返すため空センテンスが「確認済」と誤分類されるリスクがある**: `_bigrams()` が空文字列に空集合を返すとき `_overlap(set(), src)` は `1.0` を返していた。`verify_grounding()` のガード (`if not claim: continue`) により現在のコードでこの経路は通過しないが、将来のリファクタリングや直接呼び出しで空クレームが「完全に裏付けられた」として確認済に昇格するバグを引き起こし得る。防御的に `0.0` を返すよう修正 (内容のないクレームは確認不可)。

## [v0.1.44] - 2026-06-14
### Fixed
- **`suggest_questions()` が全角数字・記号リストプレフィックス (１. ２） ３、) を除去できない**: CJK 優先の LLM は番号付きリストの接頭辞として全角数字 (１２３…) や全角括弧・句読点 (）、) を出力する。`lstrip()` の文字集合が ASCII のみ (`0-9.-*)・ `) のため、全角文字が残留し質問として認識されない行が発生していた。NFKC 正規化 (`unicodedata.normalize("NFKC")`) を `lstrip()` の前に適用し全角→半角変換を行い、`lstrip` 文字集合に `、` (U+3001 列挙読点) を追加。副作用として全角疑問符 `？` → ASCII `?` に変換されるため、テスト内の期待値も合わせて更新。
